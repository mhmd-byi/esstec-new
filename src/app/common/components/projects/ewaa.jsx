      ...
      <Slider {...settings} className="h-full w-full">
        {slides.map((slide, index) => (
          <div key={index} className="relative h-full w-full">
            ...
            />
            <div className="absolute -bottom-px md:bottom-20 -ml-px md:ml-0 md:right-0 w-[19.5rem] md:w-[24.5rem] md:text-3xl bg-text-primary text-center text-bg-primary font-bold uppercase py-2 md:py-6 z-10 rounded-b-xl md:rounded-none">{slide.title}</div>
          </div>
        ))}
      </Slider>
    </div>
