      ...
      <div className={`flex flex-row flex-wrap mt-10 ${checkForScreenSize.maxWidth}`}>
        {clientData.map((data, index) => (
          <div className="basis-1/4 justify-between py-5" key={index}>
            <div className={`text-text-primary uppercase ${(checkForScreenSize.screenSize === 1536 || checkForScreenSize.screenSize === 1280 || checkForScreenSize.screenSize === 1360  || checkForScreenSize.screenSize === 1366) ? 'text-xs' : 'text-sm'}`}>
              {data.name}
            </div>
            <div className={`text-text-primary font-medium lowercase ${(checkForScreenSize.screenSize === 1536 || checkForScreenSize.screenSize === 1280 || checkForScreenSize.screenSize === 1360 || checkForScreenSize.screenSize === 1366) ? 'text-xs' : 'text-sm'}`}>
              <a
